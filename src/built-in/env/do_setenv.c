...
static bool correct_name_var(char *var)
{
    ...
    if (is_letter(*var, first_env_char_allowed) == false) {
        mini_printf_error("setenv: Variable name must begin with a letter.\n");
        return false;
    }
    for (int i = 0; var[i] != '\0'; i++)
        if (is_letter(var[i], env_char_allowed) == false) {
            mini_printf_error("setenv: Variable name must "
...
static bool is_env_var_line(char *line, char *var)
{
    char *line_dup = my_strdup(line);
    char *line_var = strtok_r(line_dup, "=", &line_dup);
...
int change_or_create(char *var, char *value, linked_list_t **head)
{
    ...
    char *line = my_strcat_inf(3, var, "=", value);
...
int do_setenv(char **args, linked_list_t **env)
{
    ...
    my_strlen(args[1]);
    if (list_len > 3) {
        mini_printf_error("setenv: Too many arguments.\n");
