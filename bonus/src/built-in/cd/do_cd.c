...
static int can_access_file(char *path)
{
    if (access(path, F_OK) == 0) {
        if (is_file(path) == 1) {
            mini_printf_error("%s: Not a directory.\n", path);
            return EPI_ERROR;
        }
...
static int update_var(linked_list_t **head, char *var)
{
...
{
    char *home = get_env_var(head, HOME_VAR);
...
int do_cd(char **args, linked_list_t **env)
{
    ...
    char *path = NULL;
    char *old_pwd = get_env_var(env, OLDPWD_VAR);

    update_var(env, OLDPWD_VAR);
    if (error_cd(env, args, len, &old_pwd) != 0)
        return COMMAND_ERROR;
    if (len == 1)
        return cd_one_args(env, old_pwd);
    if (my_strcmp(args[1], GO_OLDPWD) == 0)
        return go_old(old_pwd, env);
    path = change_for_home(args[1], env);
    if (path == NULL)
